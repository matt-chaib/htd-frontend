...
interface QuestionsShownProps {
  ...
  qotd: Question; // Question of the Day, or null if not available
}

const QuestionsShown: React.FC<QuestionsShownProps> = ({
  questions,
  showPastQuestions,
  setShowPastQuestions,
  qotd,
}) => {
  if (showPastQuestions) {
    return (
      <>
        <div className="question">
          <DailyQuestion question={qotd} />
          <Separator />
        </div>
        {questions
          .filter((question) => question.id != qotd.id)
          .map((question) => {
            return (
              <div className="question">
    ...
    return (
      <div className="question">
        ...
        <Separator />
        <button onClick={() => setShowPastQuestions(!showPastQuestions)}>
          Show Past Questions
        </button>
      </div>
    );
...
const fetchQuestions = async (limit: number): Promise<Question[]> => {
  const response = await fetch(
    `${import.meta.env.VITE_API_URL}/questions?limit=${limit}`
  );
  if (!response.ok) {
...
const fetchQOTD = async (): Promise<Question> => {
  const response = await fetch(
    `${import.meta.env.VITE_API_URL}/questions/api/question-of-the-day`
  );
  if (!response.ok) {
...
export const MainPage = () => {
  ...
  if (qotd_isError)
    return <div>Error: {qotd_error?.message || "Something went wrong"}</div>;
  ...
  return (
    <>
      ...
      <div className="questions-block">
        {qotd && <QuestionsShown
          questions={questions || []}
          showPastQuestions={showPastQuestions}
          setShowPastQuestions={setShowPastQuestions}
          qotd={qotd}
        />}
      </div>
    </>
